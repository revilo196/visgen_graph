...
fn deser_interpol<'de, D: Deserializer<'de>>(deserializer: D) -> Result<Interpolator, D::Error> {
    ...
    if u as usize >= INTERPOLATIONS.len() {
        return Err(serde::de::Error::custom("invalid interpol index"));
    }
...
impl ProgramSwitcher {
    ...
    pub fn new(prog: &Program, time: f32, store: &ParameterStore) -> Option<Self> {
        ...
        if s.last_config.len() == prog.config.len()
            && s.last_config
                .iter()
                .zip(prog.config.iter())
...
impl ProgramManager {
    ...
    pub fn update(&mut self, time: f32, store: &mut ParameterStore) {
        if let Some(up) = &self.current {
            if !up.update(time, store) {
                // current program is finished (not running)
                // use auto_next if available
                self.current = if let Some(p) = &up.prog.auto_next {
                    self.programs
                        .get(p)
                        .map(|prg| Box::new(ProgramSwitcher::new(prg, time, store).unwrap()))
                } else {
                    None
                }
            }
    ...
    fn add(&mut self, p: Pid, store: &ParameterStore) {
        ...
        let next = if next_pid > 0 {
            Some(next_pid as u32)
        } else {
            None
        };
    ...
    pub fn store(&self, path: &Path) -> Result<(), LoadStoreError> {
        let buf =
            rmp_serde::to_vec(&self.programs).map_err(LoadStoreError::SerializeError)?;

        let mut file = File::create(path).map_err(LoadStoreError::IoError)?;

        //Used for debug Human Readable format
        let file_json =
            File::create(path.with_extension("json")).map_err(LoadStoreError::IoError)?;
        serde_json::to_writer(file_json, &self.programs).expect("json error");

        file.write_all(&buf).map_err(LoadStoreError::IoError)
    }

    pub fn load(&mut self, path: &Path) -> Result<(), LoadStoreError> {
        let file = File::open(path).map_err(LoadStoreError::IoError)?;

        let loaded_programs =
            rmp_serde::from_read(file).map_err(LoadStoreError::DeserializeError)?;
...
impl Interpolate for Type {
    ...
    fn interpolation(f: f32, a: &Self, b: &Self) -> Self {
        match (a, b) {
            (Type::Int(ax), Type::Int(bx)) => {
                Type::Int(((1.0 - f) * (*ax) as f32 + f * (*bx) as f32) as i32)
            }
            (Type::Float(ax), Type::Float(bx)) => Type::Float((1.0 - f) * ax + f * bx),
            (Type::Long(ax), Type::Long(bx)) => {
                Type::Long(((1.0 - f) * (*ax) as f32 + f * (*bx) as f32) as i64)
            }
            (Type::Double(ax), Type::Double(bx)) => {
                Type::Double((1.0 - f as f64) * ax + f as f64 * bx)
            }
            _ => Type::Nil,
        }
    }
